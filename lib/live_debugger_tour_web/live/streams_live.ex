  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash}>
      ...
      <button
        ...
        class="fixed right-0 top-1/3 z-40 btn btn-primary shadow-lg rounded-r-none pl-3 pr-4 py-2 flex items-center gap-2 hover:pr-6 transition-all"
        phx-click={JS.remove_class("translate-x-full", to: "#stream-side-panel")}
      >
        <.icon name="hero-chevron-left" class="size-4" />
        <.icon name="hero-list-bullet" class="size-5" /> Preview
      </button>

      <div
        id="stream-side-panel"
        class="fixed right-0 top-0 h-full w-80 bg-base-100 shadow-2xl z-50 transform transition-transform duration-300 translate-x-full border-l border-base-300 flex flex-col"
      >
